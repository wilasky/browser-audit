...
**Fase actual:** v0.2.2 ya estaba **publicada y aprobada en la Chrome Web Store desde hace meses** (item ID `gaeakblaejmchannjngonjnmbnaeoaao`, https://chromewebstore.google.com/detail/gaeakblaejmchannjngonjnmbnaeoaao), pero el trabajo correspondiente nunca se había subido a GitHub — `main` seguía en v0.2.0 en remoto. El 2026-07-29 se reconcilió todo: se mergeó `feat/free-v03` → `main`, se subió versión **v0.2.3** (bump sobre v0.2.2: fix de guion en locale EN/ES + 2 screenshots nuevas de RGPD/diagnóstico), y **el usuario ya subió el ZIP `browser-audit-2026-07-29.zip` al CWS Developer Dashboard** — a la espera del veredicto de Google. README, landing page y PENDING_TASKS.md quedaron limpios de referencias obsoletas ("pending review", enlaces genéricos de la store, checklist de primera publicación).
**Última actualización:** 2026-07-29
**Próxima tarea:** Comprobar en el Developer Dashboard si Google aprobó v0.2.3. Si sí: actualizar el texto del "Store listing" ahí con el copy de `docs/CWS_LISTING.md` (probablemente sigue con copy antiguo) y sustituir 2 screenshots por las de `docs/screenshotsv2/`. Después, la siguiente unidad de trabajo real es retomar el roadmap: **Fase 4 (backend threat intel)** sigue sin empezar, o si se prefiere seguir en el free tier, ver "Ideas a desarrollar" más abajo (perfiles de usuario, fuentes por check, baseline dinámica) y `docs/PENDING_TASKS.md` sección "v0.2 — Features y mejoras" para el siguiente incremento (Wappalyzer-style detection, más security headers).
